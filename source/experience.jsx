/* ============================================================
   Experience layer → window.Experience
...
function Intro({ onDone }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => { if (e.key === "Enter" || e.key === "Escape" || e.key === " ") close(); };
...
function ProductionHUD({ lang }) {
  ...
  const tcRef = useRef(null), sceneRef = useRef(null), scrubRef = useRef(null), playRef = useRef(null), activeRef = useRef(-1);
  ...
  const goClip = (sel) => { const el = document.querySelector(sel); if (el) window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - 1, behavior: "smooth" }); };
...
function Reticle() {
  useEffect(() => {
    ...
    const ring = wrap.querySelector(".reticle__ring");
    const dot = wrap.querySelector(".reticle__dot");
    ...
    const onMove = (e) => {
      ...
      const focus = !!(e.target.closest && e.target.closest("a, button, input, [data-cursor]"));
...
function Lightbox({ lang }) {
  ...
  useEffect(() => {
    const onPlay = (e) => { setFilm(e.detail); document.body.style.overflow = "hidden"; };
    const onKey = (e) => { if (e.key === "Escape") close(); };
    window.addEventListener("sorya:play", onPlay);
    window.addEventListener("keydown", onKey);
    return () => { window.removeEventListener("sorya:play", onPlay); window.removeEventListener("keydown", onKey); };
  }, []);
  if (!film) return null;
...
function Experience({ lang, intro = true, editbar = true }) {
  return (
    <React.Fragment>
      {intro ? <Intro /> : null}
...
Object.assign(window, { Experience, Intro, ProductionHUD, Reticle, Lightbox, FramingGuides });
